...
var CssShortener = function CssShortener(options) {
  ...
  var replaceCss = function replaceCss(match, capturingGroup) {
    ...
    var className = capturingGroup.substr(1); // Remove dot infront of class name

    return ".".concat(t.getNewClassName(className));
  };
  ...
  this.importMap = function (map, override) {
    for (var classNameToImport in map) {
  ...
  this.getNewClassName = function (className) {
    ...
    if (t._options.ignorePrefix && className.startsWith(t._options.ignorePrefix)) return t._options.trimIgnorePrefix ? className.substr(t._options.ignorePrefix.length) : className; // Return the already mapped class name

    if (t._classNameMap[className] != null) return t._classNameMap[className]; // Generate, map and return the new class name
